...
### v2.2.0 (2025-9-11) - 本地存储歌单与移动端优化
#### 🎯 重大新功能
- **本地存储歌单** - 支持保存最多3个歌单到本地存储，快速访问常用歌单
- **分享链接解析** - 智能解析网易云音乐分享链接，自动提取歌单名称和ID
- **移动端下拉菜单** - 移动端使用下拉菜单展示保存的歌单，节省屏幕空间
- **按需加载** - 保存歌单时只存储基本信息，点击时重新获取最新数据

### v2.1.2 (2025-9-10) - 文档完善与版本对比
#### 🎨 样式修复
- **可视化音频波浪** - 降低层级优化在移动端展示的遮挡，作为背景存在
...
- **歌词显示** - 实时歌词同步高亮显示
- **专辑封面** - 动态获取和显示专辑封面
- **歌单解析** - 支持网易云音乐歌单ID和分享链接解析
- **本地存储** - 保存常用歌单到本地，支持快速访问（最多3个）
...
- **下拉菜单** - 移动端使用下拉菜单展示保存的歌单，节省屏幕空间
- **响应式交互** - 根据屏幕尺寸自动切换桌面端tab和移动端下拉菜单
...
│       ├── 图片代理         # 专辑封面代理服务
│       └── CORS处理         # 跨域请求处理
├── music.html             # 🎯 独立HTML版本：自包含音乐播放器
│   ├── 内联CSS样式         # 完整的样式定义（内联）
│   ├── 内联JavaScript      # 完整的应用逻辑（内联）
│   ├── 响应式设计          # 桌面端和移动端适配
│   ├── 移动端横滑          # 触摸手势和页面切换
│   ├── 音频可视化          # Canvas波形显示
│   └── 直接API连接         # 无需代理的API调用
